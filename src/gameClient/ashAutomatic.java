...
public class ashAutomatic {
	...
	/*
	 * public Fruits hasMostNeighbors(ArrayList<Fruits> list) { if(list.isEmpty()) {
	 * return null; } double sum; Fruits f; Point3D fruitPoint; Point3D fPoint;
	 * for(int i = 0; i < list.size(); i++) { f = list.get(0); if(!(f.isTarget())) {
	 * for(int j = i+1; j<list.size(); j++) {
	 * 
	 * } } } //f_highest.setIsTarget(true); return f_highest; }
	 */
